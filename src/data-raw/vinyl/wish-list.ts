import { PHOTOS } from "resources";
import { MUSICIANS } from "@constants";

export const PANIC_AT_THE_DISCO_AFYCSO: IAlbum = {
  musician: MUSICIANS.PANIC_AT_THE_DISCO,
  photo: PHOTOS.ALBUMS.PANIC_AT_THE_DISCO_AFYCSO,
  title: "A Fever You Can't Sweat Out",
  appleMusicId: "a-fever-you-cant-sweat-out/128167002",
  year: 2005
};

export const PARAMORE_RIOT: IAlbum = {
  musician: MUSICIANS.PARAMORE,
  photo: PHOTOS.ALBUMS.PARAMORE_RIOT,
  title: "Riot!",
  appleMusicId: "riot/604804755",
  year: 2007
};

export const KATY_PERRY_ONE_OF_THE_BOYS: IAlbum = {
  musician: MUSICIANS.KATY_PERRY,
  photo: PHOTOS.ALBUMS.KATY_PERRY_ONE_OF_THE_BOYS,
  title: "One of the Boys",
  appleMusicId: "one-of-the-boys/715853053",
  year: 2008
};

export const RIHANNA_RATED_R: IAlbum = {
  musician: MUSICIANS.RIHANNA,
  photo: PHOTOS.ALBUMS.RIHANNA_RATED_R,
  title: "Rated R",
  appleMusicId: "rated-r/338914834",
  year: 2009
};

export const LISSIE_CATCHING_A_TIGER: IAlbum = {
  musician: MUSICIANS.LISSIE,
  photo: PHOTOS.ALBUMS.LISSIE_CATCHING_A_TIGER,
  appleMusicId: "catching-a-tiger/372594767",
  title: "Catching a Tiger",
  year: 2010
};

export const BEN_HOWARD_EVERY_KINGDOM: IAlbum = {
  musician: MUSICIANS.BEN_HOWARD,
  photo: PHOTOS.ALBUMS.BEN_HOWARD_EVERY_KINGDOM,
  appleMusicId: "every-kingdom/1440804636",
  title: "Every Kingdom",
  year: 2011
};

export const NATALIA_KILLS_PERFECTIONIST: IAlbum = {
  musician: MUSICIANS.NATALIA_KILLS,
  photo: PHOTOS.ALBUMS.NATALIA_KILLS_PERFECTIONIST,
  title: "Perfectionist",
  year: 2011,
  appleMusicId: "perfectionist/1443164533"
};

export const ELLIE_GOULDING_HALCYON: IAlbum = {
  musician: MUSICIANS.ELLIE_GOULDING,
  photo: PHOTOS.ALBUMS.ELLIE_GOULDING_HALCYON,
  title: "Halcyon",
  appleMusicId: "halcyon/1442722757",
  year: 2012
};

export const LANE_8: IAlbum = {
  musician: MUSICIANS.LANE_8,
  photo: PHOTOS.ALBUMS.LANE_8_RISE,
  title: "Rise",
  appleMusicId: "rise/988732595",
  year: 2015
};

export const CARIBOU: IAlbum = {
  musician: MUSICIANS.CARIBOU,
  photo: PHOTOS.ALBUMS.CARIBOU_OUR_LOVE,
  appleMusicId: "our-love/1276611236",
  title: "Our Love",
  year: 2014
};

export const RYN_WEAVER_FOOL: IAlbum = {
  musician: MUSICIANS.RYN_WEAVER,
  title: "The Fool",
  photo: PHOTOS.ALBUMS.RYN_WEAVER_FOOL,
  year: 2015,
  appleMusicId: "the-fool/1442781012"
};

export const BEYONCE_LEMONADE: IAlbum = {
  musician: MUSICIANS.BEYONCE,
  title: "Lemonade",
  photo: PHOTOS.ALBUMS.BEYONCE_LEMONADE,
  appleMusicId: "lemonade/1460430561",
  year: 2016
};

export const TOURIST_U: IAlbum = {
  musician: MUSICIANS.TOURIST,
  appleMusicId: "u/1086160489",
  title: "U",
  photo: PHOTOS.ALBUMS.TOURIST_U,
  year: 2016
};

export const GEORGE_FITZGERALD_ALL_THAT_MUST_BE: IAlbum = {
  musician: MUSICIANS.GEORGE_FITZGERALD,
  title: "All That Must Be",
  photo: PHOTOS.ALBUMS.GEORGE_FITZGERALD_ALL_THAT_MUST_BE,
  appleMusicId: "all-that-must-be/1322664114",
  year: 2018
};

export const JON_HOPKINS_SINGULARITY: IAlbum = {
  musician: MUSICIANS.JON_HOPKINS,
  title: "Singularity",
  photo: PHOTOS.ALBUMS.JON_HOPKINS_SINGULARITY,
  appleMusicId: "singularity/1349736537",
  year: 2018
};

export const LANY_MALIBU_NIGHTS: IAlbum = {
  musician: MUSICIANS.LANY,
  title: "Malibu Nights",
  photo: PHOTOS.ALBUMS.LANY_MALIBU_NIGHTS,
  appleMusicId: "malibu-nights/1435482172",
  year: 2018
};

export const SOPHIE_OOEPUI: IAlbum = {
  musician: MUSICIANS.SOPHIE,
  title: "Oil of Every Pearl's Un-Insides",
  photo: PHOTOS.ALBUMS.SOPHIE_OOEPUI,
  appleMusicId: "oil-of-every-pearls-un-insides/1395045797",
  year: 2018
};

export const DUA_LIPA_FUTURE_NOSTALGIA: IAlbum = {
  musician: MUSICIANS.DUA_LIPA,
  title: "Future Nostalgia",
  photo: PHOTOS.ALBUMS.DUA_LIPA_FUTURE_NOSTALGIA,
  appleMusicId: "future-nostalgia/1495799403",
  year: 2020
};

export const LENNON_STELLA_THREE_TWO_ONE: IAlbum = {
  musician: MUSICIANS.LENNON_STELLA,
  title: "Three. Two. One.",
  photo: PHOTOS.ALBUMS.LENNON_STELLA_THREE_TWO_ONE,
  appleMusicId: "three-two-one/1501262697",
  year: 2020
};
